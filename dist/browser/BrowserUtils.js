...
class BrowserUtils {
        ...
        try {
            ...
            if (overlay) {
                ...
                if (rejected) {
                    ...
                    if (accepted) {
                        this.bot.logger.debug(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', '已关闭: Bing覆盖层接受');
                    }
                }
            ...
            this.bot.logger.warn(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', `处理程序错误: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async getLatestTab(page) {
        try {
            ...
            this.bot.logger.error(this.bot.isMobile, 'GET-NEW-TAB', `无法获取最新标签页: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    async reloadBadPage(page) {
        try {
            ...
            this.bot.logger.error(this.bot.isMobile, 'RELOAD-BAD-PAGE', `重新加载检查失败: ${error instanceof Error ? error.message : String(error)}`);
            return true;
        }
    }
    async closeTabs(page, config = { minTabs: 1, maxTabs: 1 }) {
        try {
            ...
            this.bot.logger.debug(this.bot.isMobile, 'SEARCH-CLOSE-TABS', `发现 ${tabs.length} 个标签页打开 (最小: ${config.minTabs}, 最大: ${config.maxTabs})`);
            // 检查是否有效
            ...
            if (tabs.length > config.maxTabs) {
                ...
                this.bot.logger.debug(this.bot.isMobile, 'SEARCH-CLOSE-TABS', `关闭了 ${closedCount}/${tabsToClose.length} 个多余的标签页以达到最大值 ${config.maxTabs}`);
                // 打开更多标签页
            }
            else if (tabs.length < config.minTabs) {
                const tabsNeeded = config.minTabs - tabs.length;
                this.bot.logger.debug(this.bot.isMobile, 'SEARCH-CLOSE-TABS', `打开 ${tabsNeeded} 个标签页以达到最小值 ${config.minTabs}`);
                const newTabPromises = Array.from({ length: tabsNeeded }, async () => {
                    try {
                        ...
                        this.bot.logger.warn(this.bot.isMobile, 'SEARCH-CLOSE-TABS', `创建新标签页失败: ${error instanceof Error ? error.message : String(error)}`);
                        return null;
                    }
                });
            ...
            this.bot.logger.error(this.bot.isMobile, 'SEARCH-CLOSE-TABS', `错误: ${error instanceof Error ? error.message : String(error)}`);
            return page;
        }
    }
    ...
    async ghostClick(page, selector, options) {
        try {
            this.bot.logger.debug(this.bot.isMobile, 'GHOST-CLICK', `尝试点击选择器: ${selector}, 选项: ${JSON.stringify(options)}`);
            // 点击前等待选择器存在
            ...
            this.bot.logger.warn(this.bot.isMobile, 'GHOST-CLICK', `${selector} 点击失败: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async disableFido(page) {
        ...
        await page.route(routePattern, route => {
            try {
                ...
                this.bot.logger.debug(this.bot.isMobile, 'DISABLE-FIDO', `修改了请求体: isFidoSupported 设置为 ${body.isFidoSupported}`);
                route.continue({
                ...
                this.bot.logger.debug(this.bot.isMobile, 'DISABLE-FIDO', `发生错误: ${error instanceof Error ? error.message : String(error)}`);
                route.continue();
            }
        });
    }
