    ...
    async handle(page, recoveryEmail) {
        try {
            ...
            for (let attempt = 1; attempt <= this.maxManualAttempts; attempt++) {
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-RECOVERY', `开始尝试 ${attempt}/${this.maxManualAttempts}`);
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-RECOVERY', `提示用户输入邮箱 (超时: ${this.maxManualSeconds}秒)`);
                ...
                if (!email) {
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `未收到或收到无效邮箱输入 (尝试 ${attempt}/${this.maxManualAttempts})`);
                ...
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `收到无效邮箱格式 (尝试 ${attempt}/${this.maxManualAttempts}) | 长度=${email.length}`);
                    if (attempt === this.maxManualAttempts) {
                ...
                if (!filled) {
                    this.bot.logger.error(this.bot.isMobile, 'LOGIN-RECOVERY', `无法填写邮箱输入字段 (尝试 ${attempt}/${this.maxManualAttempts})`);
                    if (attempt === this.maxManualAttempts) {
                ...
                const errorMessage = await (0, LoginUtils_1.getErrorMessage)(page);
                if (errorMessage) {
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `页面错误: "${errorMessage}" (尝试 ${attempt}/${this.maxManualAttempts})`);
                    if (attempt === this.maxManualAttempts) {
